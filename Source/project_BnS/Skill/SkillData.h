...
{
	Base		= 0,	// 발동 조건 없는 기본 공격
	Chain		= 1,	// 연계 스킬
	Proc		= 2,	// 막기/회피시 활성화
	Finisher	= 3,	// 특정 조건 이후 활성화되는 스킬 (3콤보 후 강한 공격)
	BossCC		= 4		// 합격기 타이밍에 활성화되는 스킬
};
...
struct FSkillDataRow : public FTableRowBase
{
	...
	ESkillSlot Slot = ESkillSlot::None;		// 입력 슬롯 (None:Enemy, 0:LMB, 1~E:Keyboard 1,2,3,4,Q,E)

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Common")
	ESkillLayer Layer = ESkillLayer::Base;	// 스킬 계층 (Layer0:조건X, Layer1:콤보체인, Layer2:막기/회피 활성화, Layer3:일정 스택/마무리, Layer4:보스 합격기)

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Common")
	int32 Priority = 0;						// 같은 Layer 내 우선 순위 수치 
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Common")
	float AnimLockSec = 0.3f;		// 스킬 사용 직후 다른 스킬 사용 불가 시간

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Common")
	float QueueWindowSec = 0.2f;	// AnimLock 이후 

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Common")
	int32 ChainNextID = -1;			// Combo (없으면 -1)

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Common")
	float ChainWindowSec = 0.f;		// Combo 연계 가능 시간

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Common")
	bool bOffLock = false;			// 회피/막기 시 Lock 무시
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Common")
	TArray<ECrowdControlType> NeedTargetCC;		// 세팅한 리스트 상태(down, stun)가 모두 타겟에 있어야 사용 가능
